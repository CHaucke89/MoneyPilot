...
DESCRIPTIONS = {
  "OpenpilotEnabledToggle": tr_noop(
    "Use the cloudypilot system for adaptive cruise control and lane keep driver assistance. " +
    "Your attention is required at all times to use this feature."
  ),
  "DisengageOnAccelerator": tr_noop("When enabled, pressing the accelerator pedal will disengage cloudypilot."),
  "LongitudinalPersonality": tr_noop(
    "Standard is recommended. In aggressive mode, cloudypilot will follow lead cars closer and be more aggressive with the gas and brake. " +
    "In relaxed mode cloudypilot will stay further away from lead cars. On supported cars, you can cycle through these personalities with " +
  ...
  "AlwaysOnDM": tr_noop("Enable driver monitoring even when cloudypilot is not engaged."),
  ...
  def __init__(self):
    ...
    for param, (title, desc, icon, needs_restart) in self._toggle_defs.items():
      ...
      if needs_restart and not locked:
        additional_desc = tr("Changing this setting will restart cloudypilot if the car is powered on.")
  ...
  def _update_toggles(self):
    ...
    if ui_state.CP is not None:
      if ui_state.has_longitudinal_control:
        ...
        long_desc = unavailable + " " + tr("cloudypilot longitudinal control may come in a future update.")
        if ui_state.CP.alphaLongitudinalAvailable:
          if self._is_release:
            long_desc = unavailable + " " + tr("An alpha version of cloudypilot longitudinal control can be tested, along with " +
                                               "Experimental mode, on non-release branches.")
          else:
            long_desc = tr("Enable the cloudypilot longitudinal control (alpha) toggle to allow Experimental mode.")
