...
class ModelWidget(Widget):
  ...
  def _render(self, rect: rl.Rectangle):
    rl.draw_rectangle_rounded(rl.Rectangle(rect.x, rect.y, rect.width, 450), 0.04, 20, rl.Color(51, 51, 51, 255))
    ...
    for line in wrapped_desc:
      text_size = measure_text_cached(desc_font, line, 40)
      line_x = rect.x + (rect.width - text_size.x) // 2
      rl.draw_text_ex(desc_font, line, rl.Vector2(int(line_x), int(y)), 40, 0, rl.WHITE)
      y += 40 * FONT_SCALE
