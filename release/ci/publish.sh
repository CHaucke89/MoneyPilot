...
# "Tagging"
echo "#define CLOUDYPILOT_VERSION \"$VERSION\"" > ${OUTPUT_DIR}/cloudypilot/common/version.h
...
git fetch origin $DEV_BRANCH || (git checkout -b $DEV_BRANCH && git commit --allow-empty -m "cloudypilot v$VERSION release" && git push -u origin $DEV_BRANCH)
...
version: cloudypilot v$CP_VERSION (${EXTRA_VERSION_IDENTIFIER})
date: $DATETIME
